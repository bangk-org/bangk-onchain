// File: bangk-onchain-common/src/pda.rs
...
use crate::{debug, errors::BangkError};
...
pub trait BangkPda: BorshDeserialize + BorshSerialize {
    ...
    fn write<'a>(&self, account: &AccountInfo<'a>, payer: &AccountInfo<'a>) -> ProgramResult {
        let mut account_data = borsh::to_vec(self).map_err(|_err| BangkError::InvalidRawData)?;
    ...
    fn create<'a>(
        ...
        let mut data = borsh::to_vec(self).map_err(|_err| BangkError::InvalidRawData)?;
    ...
    fn delete<'a>(&self, account: &AccountInfo<'a>, payer: &AccountInfo<'a>) -> ProgramResult {
        ...
        **payer.lamports.borrow_mut() = account
            ...
            .ok_or(BangkError::RentExemptionRetrieval)?;
...
pub fn from_account<T>(account: &AccountInfo) -> Result<T, ProgramError>
where
    T: BorshDeserialize + BangkPda,
{
    let data = account.try_borrow_data()?;
    let res = T::try_from_slice(&data)?;
    if !res.is_valid() {
        return Err(BangkError::InvalidPdaType.into());
    }
    Ok(res)
}
