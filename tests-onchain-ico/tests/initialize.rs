...
// -----
// Last modified: Tuesday 25 June 2024 @ 15:52:00
...
async fn default() {
    ...
    let (config_pda, _) = IcoConfigurationPda::get_address();
    let (admin_pda, _) = MultiSigPda::get_address(MultiSigType::Admin, &env.program_id);

    // Testing configuration PDA integrity
    let Some(config): Option<IcoConfigurationPda> = env.from_account(&config_pda).await else {
