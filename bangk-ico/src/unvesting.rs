...
// -----
// Last modified: Tuesday 18 June 2024 @ 19:47:37
// Modified by: Vincent Berthier
// -----
// Copyright © 2024 <Bangk> - All rights reserved

use bangk_onchain_common::errors::BangkError;
...
impl UnvestingScheme {
    ...
    pub fn unvested(&self, launch: i64, now: i64) -> Result<u64, BangkError> {
        const WEEK_S: i64 = 86_400 * 7;
        let weeks = u8::try_from(
            (now.checked_sub(launch).ok_or(BangkError::ArithmeticError)?)
                .checked_div(WEEK_S)
                .ok_or(BangkError::ArithmeticError)?,
        )
        .map_err(|_err| BangkError::ArithmeticError)?;
        if weeks < self.start {
            Ok(0_u64)
...
mod tests {
    ...
    fn now_before_launch() {
        ...
        assert_eq!(
            scheme.unvested(0, now).unwrap_err(),
            BangkError::ArithmeticError
        );
    }
