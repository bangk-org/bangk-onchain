...
// -----
// Last modified: Tuesday 25 June 2024 @ 15:49:49
...
use bangk_onchain_common::{
    check_ata_exists, check_pda_owner, check_signers, debug,
    errors::BangkError,
    get_ata_owner, get_timestamp,
    pda::{from_account, BangkPda},
...
use crate::{
    ...
    investment::{UserIcoInvestment, UserInvestment, UserInvestmentPda},
...
impl<'a> InitializeAccounts<'a> {
    fn new(accounts: &[AccountInfo<'a>]) -> Result<Self, ProgramError> {
        ...
        Ok(Self {
            ...
            system_program: next_account_info(accounts_iter)?.clone(),
        })
    }
}
...
fn initialize(
    ...
    if *ctx.bangk.key != INIT_KEY {
        ...
        return Err(BangkError::InvalidSigner.into());
    }
    ...
    let config = IcoConfigurationPda::new(args.config_bump, &args.unvesting, ctx.admin_sig.key);
...
struct MintCreationAccounts<'a> {
    ...
    #[allow(dead_code)]
    admin2: AccountInfo<'a>,
    #[allow(dead_code)]
    admin3: AccountInfo<'a>,
...
impl<'a> MintCreationAccounts<'a> {
    fn new(accounts: &[AccountInfo<'a>]) -> Result<Self, ProgramError> {
        ...
        Ok(Self {
            ...
            admin2: next_account_info(accounts_iter)?.clone(),
            admin3: next_account_info(accounts_iter)?.clone(),
...
fn mint_creation(
    ...
    let mint_len =
        ExtensionType::try_calculate_account_len::<Mint>(&[ExtensionType::MetadataPointer])
            .map_err(|_err| BangkError::CrossProgramCallFailed)?;

    let metadata = TokenMetadata {
        update_authority: OptionalNonZeroPubkey::try_from(Some(*ctx.sig_admin.key))?,
    ...
    let meta_len = metadata
        .tlv_size_of()
        .map_err(|_err| BangkError::InvalidRawData)?;

    let data_len = mint_len
        .checked_add(meta_len)
        .ok_or(BangkError::IntegerOverflow)?;
    debug!(
...
fn user_investment(
    ...
    if ctx.investment.lamports() == 0 {
        ...
        pda.investment.investments.push(UserIcoInvestment {
...
fn cancel_investment(
    ...
    if config.launch_date > 0 {
        return Err(BangkError::CancelIcoInvestmentAfterLaunch.into());
    }

    // If PdA doesn't exist that's an error
    if ctx.investment.lamports() == 0 {
        return Err(BangkError::ATADoesNotExist.into());
    }

    let mut pda: UserInvestmentPda = from_account(&ctx.investment)?;
    let mut amount = args.amount;
    // Investments that won’t be touched
    let mut investments: Vec<UserIcoInvestment> = pda
        .investment
        .investments
        .iter()
    ...
    if amount > 0 {
        return Err(BangkError::InvalidAmount.into());
    }
...
fn launch_bgk(program_id: &Pubkey, accounts: &[AccountInfo], args: LaunchBGKArgs) -> ProgramResult {
    ...
    let mut config: IcoConfigurationPda = from_account(&ctx.config)?;

    if config.launch_date > 0 {
        return Err(BangkError::BGKTokenAlreadyLaunched.into());
    }
    if ctx.ata_invested.lamports() > 0 {
        msg!("Invested ATA already exists: aborting to prevent any risk.");
        return Err(BangkError::AccountAlreadyExists.into());
    }
...
fn vesting_release(program_id: &Pubkey, accounts: &[AccountInfo]) -> ProgramResult {
    ...
    if config.launch_date == 0 {
        return Err(BangkError::IcoUnvestBeforeLaunch.into());
    }
    check_ata_exists!(ctx.investment);
    if investment.investment.user != *ctx.user.key {
        return Err(BangkError::AccountOwnerMismatch.into());
    }
    ...
    for invest in &mut investment.investment.investments {
        ...
        let rule = match invest.custom_rule {
            ...
            None => *config
                .unvesting
                .get(&invest.kind)
                .ok_or(BangkError::InvalidUnvestingDefinition)?,
        };
