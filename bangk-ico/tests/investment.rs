...
use bangk_ico::{user_investment, UnvestingScheme, UnvestingType, UserInvestmentPda};
use bangk_onchain_common::{pda::PdaType, Error};
...
async fn invalid_custom_scheme() {
    ...
    assert!(
        res1.is_err_and(|err| err == Error::InvalidUnvestingDefinition),
        "there was an unexpected error in the instruction"
    );
    ...
    let res2 = env.execute_transaction(&[instruction2], &["API"]).await;
    assert!(
        res2.is_err_and(|err| err == Error::InvalidUnvestingDefinition),
        "there was an unexpected error in the instruction"
    );
}
